...
emr = EmojiWordReader('EmoticonLookupTable.txt')
...
def has_emoji(tweet):
    dweebs = map(lambda x : nwr.has_emoji(x[0]),tweet)
    return reduce(lambda a,x: a or x , dweebs )


def posEmojiScore(tweet,posScore):
    for word in tweet:
        if emr.has_emoji(word[0]):
            score = emr.get_score(word[0])
            if score > 0:
                posScore = posScore + 1

    return posScore

def negEmojiScore(tweet,negScore):
    for word in tweet:
        if emr.has_emoji(word[0]):
            score = emr.get_score(word[0])
            if score < 0:
...
def get_sentiment_from_tweet(tweet):
    ...
    posScore = posEmojiScore(tweet,posScore)
    negScore = negEmojiScore(tweet,negScore)
